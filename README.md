...
2. Create an API Key and Secret in your oklink account at oklink.com.
3. In the admin control panel, go to "Setup" > "Payment Gateways", select
   "Oklink" in the list of modules and click Activate.
4. Enter your API Key and Secret from step 2. 
5. Click "Save Changes."

# Usage

When a client chooses the Oklink payment method, they will be presented with an
invoice showing a button to pay the order.  Upon requesting to pay their order,
the system takes the client to a oklink.com invoice page where the client is
presented with oklink payment instructions.  Once payment is received, a link
...
order made via Oklink ("Orders" > "Pending Orders").  This screen will tell
you whether payment has been received by the oklink servers.  
